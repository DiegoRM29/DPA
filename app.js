...
const App = (() => {
  ...
  let MAPPING = null;
  let DATASET = null;
  let charts = {};   // registry de TODOS los charts activos
  let recFilter = 'all';
  ...
  // ============ RECOMMENDATION ENGINE (revised) ============
  // Devuelve { accion, pct, razon, tipo }
  function recommendAction(s) {
    ...
    if (mg < 0.05) return {
      ...
      razon: `Margen crítico ${mgPct}%. Renegociar con proveedor o evaluar discontinuar.`
    };
    if (mg < 0.10 && absE > 1) return {
      accion: 'REVISAR COSTO', pct: 0,
      razon: `Margen ${mgPct}% bajo combinado con elasticidad ${eFmt}. Reformular oferta antes de tocar precio.`
    };

    // 2. PROMOCIONES — elasticidad alta + margen suficiente
    if (absE > 2.0 && mg > 0.45) return {
      accion: 'PROMO 2X1', pct: -50,
      razon: `Elasticidad ${eFmt} muy alta · margen ${mgPct}% absorbe 2x1. Multiplica volumen sin pérdida unitaria.`
    };
    if (absE > 1.8 && mg > 0.38) return {
      accion: 'PROMO 3X2', pct: -33,
      razon: `Demanda muy sensible (${eFmt}). 3x2 dispara volumen incremental con margen suficiente para absorber.`
    };
    if (absE > 1.5 && mg > 0.30) return {
      accion: 'PROMO 4X3', pct: -25,
      razon: `Producto elástico (${eFmt}). Promo de menor profundidad (4x3) preserva margen y activa rotación.`
    };
    if (absE > 1.3 && mg > 0.25) return {
      accion: 'BUNDLE', pct: -10,
      razon: `Sensibilidad media-alta. Bundle con producto complementario sube ticket sin descuento directo.`
    };
    if (absE > 1.2 && mg > 0.20) return {
      accion: 'BAJAR PRECIO', pct: -5,
      razon: `Elasticidad ${eFmt} indica respuesta de volumen a bajada moderada de precio.`
    };

    // 3. EVITAR PROMO — elasticidad alta sin margen para absorber
    if (absE > 1.2 && mg < 0.18) return {
      accion: 'EVITAR PROMO', pct: 0,
      razon: `Elast ${eFmt} alta sin margen para absorber descuento. Promo destruye utilidad neta.`
    };

    // 4. SUBIR PRECIO — escala con margen disponible y baja elasticidad
    if (absE < 0.3 && mg > 0.35) return {
      accion: 'SUBIR PRECIO', pct: 12,
      razon: `Demanda prácticamente inelástica (${eFmt}). Margen amplio (${mgPct}%) permite captura agresiva.`
    };
    if (absE < 0.5 && mg > 0.30) return {
      accion: 'SUBIR PRECIO', pct: 8,
      razon: `Elasticidad muy baja (${eFmt}) · margen ${mgPct}%. Alta oportunidad de captura de margen.`
    };
    if (absE < 0.6 && mg > 0.25) return {
      accion: 'SUBIR PRECIO', pct: 5,
      razon: `Demanda poco sensible (${eFmt}). Incremento moderado sin afectar volumen relevante.`
    };
    if (absE < 0.7 && mg > 0.20) return {
      accion: 'SUBIR PRECIO', pct: 3,
      razon: `Elasticidad baja (${eFmt}) compensa el margen ${mgPct}%. Ajuste conservador justificado.`
    };
    if (absE < 0.5 && mg > 0.15) return {
      accion: 'SUBIR PRECIO', pct: 2,
      razon: `Elasticidad ${eFmt} baja pero margen ajustado. Incremento prudente para mejorar utilidad.`
    };

    // 5. Segmentos específicos
    if (s.segmento === 'Hero Product') return {
      accion: 'MANTENER', pct: 0,
      razon: 'Producto estrella · revenue y margen en balance óptimo. Defender posición sin cambios.'
    };
    if (s.segmento === 'Premium Product' && mg > 0.30) return {
      accion: 'CROSS-SELL', pct: 0,
      razon: 'Margen alto pero volumen limitado. Empujar via cross-sell desde productos de tráfico.'
    };
    if (s.segmento === 'Traffic Driver') return {
      accion: 'MANTENER', pct: 0,
      razon: 'Producto de tráfico. Usar como gancho para cross-sell de premium.'
    };

    // 6. Confianza baja
    if (conf === 'Baja' && Math.random() < 0.5) return {
      accion: 'A/B TEST', pct: 0,
      razon: `Baja confianza estadística en elasticidad (R²<0.2). Probar variaciones de precio en muestra controlada.`
    };

    // 7. Default
    return {
      accion: 'MANTENER', pct: 0,
      razon: 'Performance estable, sin oportunidad clara identificada. Monitorear.'
    };
  }
  ...
  const actionPill = {
    ...
    'EVITAR PROMO': 'pill-yellow', 'REVISAR COSTO': 'pill-red',
  ...
  function renderDescriptive() {
    ...
    if (pp) {
      charts.postPromo = new Chart(document.getElementById('chartPostPromo'), {
        type: 'line',
        data: {
      ...
      document.getElementById('chartPostPromo').parentElement.innerHTML = '<div style="display:grid;place-items:center;height:100%;color:var(--text-3);font-size:12px;text-align:center;padding:20px;">Insuficiente variación de precio temporal para detectar promociones</div>';
  ...
  function renderElasticity() {
    ...
    const avg = skus.reduce((a,s) => a + s.elasticidad, 0) / Math.max(skus.length,1);
  ...
  function updateSim() {
    ...
    else if (totalProfitImpact > baseProfit * 0.05) reco = `<span style="color:var(--green);font-weight:600;">✓ Escenario favorable.</span> Impacto en utilidad: ${fmt.signed((totalProfitImpact/baseProfit)*100)}. La elasticidad (${sku.elasticidad.toFixed(2)}) permite esta acción sin destruir volumen.`;
    ...
    // ===== CURVA DEL SIMULADOR (ahora SÍ incorpora costo + promo + marker) =====
    destroyChart('sim');
    const pts = [];
    const promoFactor = 1 - promo;
    ...
    const currentProf = (newPrice - newCost) * currentVol;
    ...
    charts.sim = new Chart(document.getElementById('chartSim'), {
      ...
      data: {
        datasets: [
          ...
          { label: 'Escenario actual', type: 'scatter',
